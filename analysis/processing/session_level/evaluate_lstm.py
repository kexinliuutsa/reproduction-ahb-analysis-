...
if __name__ == "__main__":
    ...
    results: Dict[str, LSTMClassificationResult] = {}
    ...
    for method_name, session_generator in generators_in_comparison.items():
        ...
        result = classify_using_lstm(FEATURE_LENGTH, total_sessions, total_labels)
        results[method_name] = result

    timestamp = generate_timestamp()

    # %% simply dump the pkl
    with open(f"lstm_classification_results_{timestamp}.pkl", "wb") as f:
        pickle.dump(results, f)

    with open(f"lstm_classification_results_{timestamp}.pkl", "rb") as f:
    ...
    print(df_results)
    df_results.to_csv(f"lstm_classification_results_{timestamp}.csv", index=False)